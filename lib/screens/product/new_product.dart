...
class _NewProductState extends State<NewProduct> {
  ...
  getCategories() async {
    categoryResponse = await ProductRepository().getCategoryRes();
  ...
  bool requiredFieldVerification() {
    print(choiceAttributes.toString());
    print(selectedAttributes.toString());
    print(productVariations.toString());
    if (productNameEditTextController.text.trim().toString().isEmpty) {
      ...
      return false;
    } else if (isColorActive&&(selectedColors==null||selectedColors!.isEmpty)) {
      // print(isColorActive.toString());

      // print(selectedColors.toString());
      ToastComponent.showDialog("Colors Required", gravity: Toast.center);
      return false;
    } else if (selectedAttributes.isNotEmpty) {
      // print(selectedAttributes.toString());
      // print(productVariations.toString());
      if(productVariations.isEmpty) {
        ToastComponent.showDialog("attribute Required", gravity: Toast.center);
  ...
  submitProduct(String button) async {
    ...
    Map postValue = Map();
    postValue.addAll({
      // "_token":"7|WUmC0mMgX4nMPhHaferLBPuMsZ9B1uzSsMAy0mWh",
      // "added_by":"seller",
      "name": productName,
      "fq_bought_product_category_id": mainCategoryId,
    ...
    return SingleChildScrollView(
      ...
      child: Row(
        children: [
          ...
          // tabBarDivider(),
          buildTopTapBarItem(
              LangText(context: context).getLocal()!.shipping_ucf, 3),
